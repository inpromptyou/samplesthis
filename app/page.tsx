"use client";

import Link from "next/link";
import Nav from "@/components/Nav";
import Footer from "@/components/Footer";
import ScrollReveal from "@/components/ScrollReveal";

const SAMPLE_JOBS = [
  { app: "Fitness tracking app", audience: "Gym-goers, 25-40", testers: 5, budget: 12, time: "2h ago", applied: 8 },
  { app: "SaaS onboarding flow", audience: "Small business owners", testers: 3, budget: 15, time: "45m ago", applied: 5 },
  { app: "Crypto portfolio tracker", audience: "Active traders", testers: 8, budget: 8, time: "1h ago", applied: 12 },
  { app: "Recipe sharing mobile app", audience: "Home cooks, 30-55", testers: 5, budget: 10, time: "3h ago", applied: 9 },
];

export default function Home() {
  return (
    <>
      <Nav />
      <main>

        {/* ═══ HERO — Mercor style: clean, centered, stats inline ═══ */}
        <section className="pt-32 pb-20 px-6">
          <div className="max-w-[800px] mx-auto text-center">
            {/* Stats bar — Mercor style: inline, plain */}
            <div className="flex items-center justify-center gap-6 md:gap-10 mb-10 hero-anim ha-1">
              {[
                { label: "Average delivery", value: "4hr" },
                { label: "Satisfaction", value: "98%" },
                { label: "Avg per tester", value: "$8" },
              ].map((s) => (
                <div key={s.label} className="flex items-center gap-2">
                  <span className="text-[13px] text-[var(--text-dim)]">{s.label}</span>
                  <span className="h text-[13px] font-bold text-[var(--text)]">{s.value}</span>
                </div>
              ))}
            </div>

            <h1 className="h text-[clamp(2.8rem,7vw,5rem)] font-bold leading-[1.0] tracking-[-0.035em] mb-6 hero-anim ha-2">
              Find the flinch before your users do
            </h1>

            <p className="text-[17px] text-[var(--text-muted)] max-w-[480px] mx-auto mb-10 leading-[1.6] hero-anim ha-3">
              Real humans matched to your audience test your app and find every
              friction point. Set your own budget, pay per tester.
            </p>

            <div className="flex items-center justify-center gap-3 hero-anim ha-4">
              <Link href="/become-a-tester" className="btn btn-primary btn-lg">Start testing</Link>
              <Link href="/submit" className="btn btn-outline btn-lg">Post a job</Link>
            </div>
          </div>
        </section>

        {/* ═══ LIVE JOBS — Mercor "Latest roles" style grid ═══ */}
        <section className="pb-24 px-6">
          <div className="max-w-[1000px] mx-auto">
            <div className="flex items-center justify-between mb-6">
              <h2 className="h text-[15px] font-semibold text-[var(--text)]">Latest jobs</h2>
              <Link href="/submit" className="text-[13px] text-[var(--text-dim)] hover:text-[var(--text)] transition-colors flex items-center gap-1">
                View all
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M9 18l6-6-6-6" /></svg>
              </Link>
            </div>
            <div className="grid sm:grid-cols-2 gap-4">
              {SAMPLE_JOBS.map((job) => (
                <div key={job.app} className="card p-5">
                  <div className="flex items-start justify-between mb-4">
                    <div>
                      <h3 className="h text-[15px] font-semibold text-[var(--text)]">{job.app}</h3>
                      <p className="text-[13px] text-[var(--text-muted)] mt-0.5">${job.budget}/tester</p>
                    </div>
                    <span className="text-[12px] text-[var(--text-dim)]">{job.time}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      {/* Avatar stack — like Mercor */}
                      <div className="flex -space-x-2">
                        {Array.from({ length: Math.min(3, job.applied) }).map((_, i) => (
                          <div key={i} className="w-6 h-6 rounded-full bg-gradient-to-br from-orange-200 to-amber-100 border-2 border-white" />
                        ))}
                      </div>
                      <span className="text-[12px] text-[var(--text-dim)]">{job.applied} applied</span>
                    </div>
                    <span className="text-[12px] font-medium text-[var(--accent)]">Apply</span>
                  </div>
                </div>
              ))}
            </div>
          </div>
        </section>

        {/* ═══ FEATURE BLOCK — Mercor APEX-style gradient block ═══ */}
        <section className="px-6 pb-24">
          <div className="max-w-[1000px] mx-auto">
            <div className="feature-block px-10 py-16 md:px-16 md:py-20 text-center">
              <h2 className="h text-3xl md:text-[3.5rem] font-bold tracking-[-0.03em] leading-tight mb-5">
                Flinch Report
              </h2>
              <p className="text-[16px] text-white/70 max-w-md mx-auto mb-8 leading-relaxed">
                Screen recordings, bug reports, friction notes, and a flinch score for every tester — delivered to your inbox.
              </p>
              <Link href="/submit" className="btn bg-white text-[var(--dark)] hover:bg-white/90 btn-lg">
                Get your report
              </Link>
            </div>
          </div>
        </section>

        {/* ═══ TWO SIDES — clean minimal ═══ */}
        <section className="py-20 px-6">
          <div className="max-w-[1000px] mx-auto">
            <ScrollReveal>
              <div className="text-center mb-14">
                <h2 className="h text-2xl md:text-[2.5rem] font-bold tracking-[-0.03em] text-[var(--text)]">
                  One platform. Two ways to win.
                </h2>
              </div>
            </ScrollReveal>

            <div className="grid md:grid-cols-2 gap-5">
              <ScrollReveal delay={100}>
                <Link href="/submit" className="card block p-8 group">
                  <div className="w-10 h-10 rounded-xl grad-warm-subtle flex items-center justify-center mb-5">
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#F97316" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
                      <path d="M21 16V8a2 2 0 00-1-1.73l-7-4a2 2 0 00-2 0l-7 4A2 2 0 003 8v8a2 2 0 001 1.73l7 4a2 2 0 002 0l7-4A2 2 0 0021 16z" />
                    </svg>
                  </div>
                  <h3 className="h text-lg font-semibold mb-2 text-[var(--text)]">I built something</h3>
                  <p className="text-[14px] text-[var(--text-muted)] leading-[1.7] mb-5">
                    Post a test job with your budget. Describe your ideal user. Get screen recordings, bug reports, and every flinch moment.
                  </p>
                  <span className="text-[13px] font-medium text-[var(--accent)] group-hover:underline">Post a test job →</span>
                </Link>
              </ScrollReveal>

              <ScrollReveal delay={200}>
                <Link href="/become-a-tester" className="card block p-8 group">
                  <div className="w-10 h-10 rounded-xl grad-warm-subtle flex items-center justify-center mb-5">
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#F97316" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
                      <path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2" /><circle cx="9" cy="7" r="4" /><path d="M23 21v-2a4 4 0 00-3-3.87M16 3.13a4 4 0 010 7.75" />
                    </svg>
                  </div>
                  <h3 className="h text-lg font-semibold mb-2 text-[var(--text)]">I want to earn</h3>
                  <p className="text-[14px] text-[var(--text-muted)] leading-[1.7] mb-5">
                    Browse jobs matched to your profile. Use apps for 15 minutes, give honest feedback, get paid same day.
                  </p>
                  <span className="text-[13px] font-medium text-[var(--accent)] group-hover:underline">Sign up as a tester →</span>
                </Link>
              </ScrollReveal>
            </div>
          </div>
        </section>

        {/* ═══ HOW IT WORKS — minimal steps ═══ */}
        <section className="warm-section py-20 px-6">
          <div className="max-w-[800px] mx-auto">
            <ScrollReveal>
              <h2 className="h text-2xl md:text-[2.5rem] font-bold tracking-[-0.03em] text-center mb-16 text-[var(--text)]">
                How it works
              </h2>
            </ScrollReveal>

            <div className="grid md:grid-cols-2 gap-14">
              <div>
                <p className="h text-[12px] font-semibold text-[var(--accent)] uppercase tracking-[0.15em] mb-6">For businesses</p>
                {[
                  { t: "Paste your URL", d: "Drop your app link and tell us what to test." },
                  { t: "Describe your user", d: "\"Women 25-40 who shop online\" — we match from our pool." },
                  { t: "Set your budget", d: "Pay what you want per tester. No fixed plans." },
                  { t: "Get your flinch report", d: "Screen recordings, bug reports, friction notes — in your inbox." },
                ].map((s, i) => (
                  <ScrollReveal key={s.t} delay={i * 80}>
                    <div className="flex gap-4 mb-6">
                      <span className="h text-[12px] font-bold text-[var(--text-dim)] mt-0.5">{String(i + 1).padStart(2, "0")}</span>
                      <div>
                        <h4 className="h text-[14px] font-semibold mb-0.5 text-[var(--text)]">{s.t}</h4>
                        <p className="text-[13px] text-[var(--text-muted)] leading-relaxed">{s.d}</p>
                      </div>
                    </div>
                  </ScrollReveal>
                ))}
              </div>

              <div>
                <p className="h text-[12px] font-semibold text-[#EF4444] uppercase tracking-[0.15em] mb-6">For testers</p>
                {[
                  { t: "Tell us about you", d: "Age, interests, devices. 60 seconds, no interviews." },
                  { t: "Browse jobs", d: "We match you to test jobs that fit your profile." },
                  { t: "Use the app for 15 min", d: "Record your screen, note what feels off." },
                  { t: "Get paid same day", d: "$5-20+ per test via PayPal or bank transfer." },
                ].map((s, i) => (
                  <ScrollReveal key={s.t} delay={i * 80}>
                    <div className="flex gap-4 mb-6">
                      <span className="h text-[12px] font-bold text-[var(--text-dim)] mt-0.5">{String(i + 1).padStart(2, "0")}</span>
                      <div>
                        <h4 className="h text-[14px] font-semibold mb-0.5 text-[var(--text)]">{s.t}</h4>
                        <p className="text-[13px] text-[var(--text-muted)] leading-relaxed">{s.d}</p>
                      </div>
                    </div>
                  </ScrollReveal>
                ))}
              </div>
            </div>
          </div>
        </section>

        {/* ═══ DELIVERABLES — 4 cards ═══ */}
        <section className="py-20 px-6">
          <div className="max-w-[1000px] mx-auto">
            <ScrollReveal>
              <h2 className="h text-2xl md:text-[2.5rem] font-bold tracking-[-0.03em] text-center mb-12 text-[var(--text)]">
                What you get
              </h2>
            </ScrollReveal>

            <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {[
                { icon: "M14.752 11.168l-3.197-2.132A1 1 0 0010 9.87v4.263a1 1 0 001.555.832l3.197-2.132a1 1 0 000-1.664zM21 12a9 9 0 11-18 0 9 9 0 0118 0z", title: "Screen recordings", desc: "Full sessions. See where users flinch." },
                { icon: "M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4", title: "Bug reports", desc: "Steps to reproduce, severity, device." },
                { icon: "M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z", title: "Friction notes", desc: "What felt awkward or confusing." },
                { icon: "M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z", title: "Flinch score", desc: "Ship when you hit zero." },
              ].map((d, i) => (
                <ScrollReveal key={d.title} delay={i * 60}>
                  <div className="card p-5 h-full">
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#F97316" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="mb-4 opacity-60"><path d={d.icon} /></svg>
                    <h3 className="h text-[14px] font-semibold mb-1 text-[var(--text)]">{d.title}</h3>
                    <p className="text-[12px] text-[var(--text-muted)] leading-[1.7]">{d.desc}</p>
                  </div>
                </ScrollReveal>
              ))}
            </div>
          </div>
        </section>

        {/* ═══ BUDGET GUIDE ═══ */}
        <section className="dark-section py-20 px-6">
          <div className="max-w-[800px] mx-auto text-center">
            <ScrollReveal>
              <h2 className="h text-2xl md:text-[2.5rem] font-bold tracking-[-0.03em] mb-3 text-white">
                You set the price
              </h2>
              <p className="text-[15px] text-white/40 mb-12 max-w-md mx-auto">
                No fixed plans. Higher budgets attract testers faster.
              </p>
            </ScrollReveal>

            <div className="grid md:grid-cols-3 gap-4">
              {[
                { range: "$5–8", speed: "~24h", label: "Economy" },
                { range: "$10–15", speed: "~4h", label: "Standard", pop: true },
                { range: "$20+", speed: "~1h", label: "Priority" },
              ].map((t, i) => (
                <ScrollReveal key={t.label} delay={i * 80}>
                  <div className={`rounded-2xl border p-6 text-center ${t.pop ? "border-orange-500/20 bg-white/[0.04]" : "border-white/[0.06] bg-white/[0.02]"}`}>
                    {t.pop && <span className="h inline-block text-orange-400 text-[10px] font-bold mb-2 uppercase tracking-wider">Recommended</span>}
                    <p className="h text-2xl font-bold text-white mb-1">{t.range}</p>
                    <p className="text-[12px] text-white/30 mb-0.5">per tester</p>
                    <p className="text-[12px] text-orange-400/70">{t.speed} pickup</p>
                  </div>
                </ScrollReveal>
              ))}
            </div>
          </div>
        </section>

        {/* ═══ MARQUEE ═══ */}
        <section className="py-12 overflow-hidden border-b border-black/[0.04]">
          <p className="h text-[10px] font-medium text-[var(--text-dim)] text-center uppercase tracking-[0.25em] mb-5">Built for makers who ship with</p>
          <div className="flex items-center marquee whitespace-nowrap">
            {[...Array(2)].flatMap(() =>
              ["Cursor", "Bolt", "Replit", "Lovable", "v0", "Windsurf", "Claude", "ChatGPT", "Vercel", "Supabase"].map((t, i) => (
                <span key={`${t}-${i}`} className="mx-10 h text-[13px] font-semibold text-black/25 uppercase tracking-[0.12em]">{t}</span>
              ))
            )}
          </div>
        </section>

        {/* ═══ CTA — simple ═══ */}
        <section className="py-24 px-6 text-center">
          <ScrollReveal>
            <h2 className="h text-2xl md:text-[3rem] font-bold tracking-[-0.03em] leading-tight mb-5 text-[var(--text)]">
              Find the flinch.<br />Ship with confidence.
            </h2>
            <p className="text-[16px] text-[var(--text-muted)] mb-8 max-w-md mx-auto">
              Real humans. Your audience. Every friction point.
            </p>
            <div className="flex items-center justify-center gap-3">
              <Link href="/become-a-tester" className="btn btn-primary btn-lg">Start testing</Link>
              <Link href="/submit" className="btn btn-outline btn-lg">Post a job</Link>
            </div>
          </ScrollReveal>
        </section>
      </main>
      <Footer />
    </>
  );
}
